...
### 7. Building and Deploying the Android APK

This section describes how we packaged UConnect into a testable Android APK using Expo EAS Build, following the course deployment requirements.

#### EAS Account and CLI Setup

1. Create or log into an Expo account at: https://expo.dev  
2. Install the EAS CLI globally (one-time setup):

   ```bash
   npm install -g eas-cli
   ```

3. Log in and verify the account:
...
The project already includes an `eas.json` file with development, preview, and production profiles. For our final submission we use the **preview** profile, which produces an installable test build without dev tools.

#### Environment Variables

Locally, each developer keeps Supabase credentials in a `.env` file:
...
On Expo, the same keys are configured as **EAS project environment variables** so remote builds can connect to Supabase without exposing secrets in the repository.

#### Triggering the Android Build
...
EAS uploads the project, installs dependencies, bundles the app using the Expo SDK configuration from `app.json` and `index.ts`, and then builds an Android APK on Expo's servers.

#### Downloading and Testing the APK

After the build finishes, Expo provides a build page URL containing the downloadable APK file. The APK can be installed on compatible Android devices or emulators (with "install from unknown sources" enabled) and is used by the TAs to grade the project.
